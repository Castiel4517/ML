...
joblib.dump(meta_clf, os.path.join(OUTDIR, "meta_xgb_v3.pkl"))
print("Saved meta_xgb_v3.pkl")
...
# -------------------------
try:
    imp = meta_clf.get_booster().get_score(importance_type='gain')
    imp_df = pd.DataFrame({
        "feature": list(imp.keys()),
        "gain": list(imp.values())
    }).sort_values("gain", ascending=False)
    imp_df['model'] = imp_df['feature'].apply(lambda x: base_names[int(x[1:])])
    imp_df = imp_df.groupby('model')['gain'].sum().reset_index().sort_values('gain', ascending=False)
